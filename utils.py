...
def save_checkpoint(model: Hidden, epoch: int, checkpoint_folder: str, training_losses):
# def save_checkpoint(model: Hidden, experiment_name: str, epoch: int, checkpoint_folder: str):
    ...
    # checkpoint_filename = "HiDDeN_32_epoch%2d_training_loss%.2f%%.pth" %(epoch, training_losses)
    checkpoint_filename = f'--epoch-{epoch}.pyt'
    ...
    checkpoint = {
        'encoder': model.encoder_decoder.encoder.state_dict(),
        'decoder': model.encoder_decoder.decoder.state_dict(),
        'noiser' : model.encoder_decoder.noiser.state_dict(),
...
def model_from_checkpoint(hidden_net, checkpoint):
    ...
    hidden_net.encoder_decoder.encoder.load_state_dict(checkpoint['encoder'])
    hidden_net.encoder_decoder.decoder.load_state_dict(checkpoint['decoder'])
...
def get_data_loaders(hidden_config: HiDDenConfiguration, train_options: TrainingOptions):
    ...
    train_loader = torch.utils.data.DataLoader(train_images, batch_size=train_options.batch_size, shuffle=True,drop_last=True,
                                               num_workers=0)

    me_train_images = datasets.ImageFolder(train_options.me_train_folder, data_transforms['train'])
    me_train_loader = torch.utils.data.DataLoader(me_train_images, batch_size=train_options.batch_size, shuffle=True,
                                               drop_last=True,
                                               num_workers=0)
    ...
    return train_loader,me_train_loader
